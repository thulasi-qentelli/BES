...
class FeedTableCell: UITableViewCell {
    ...
    func configure(with feedDetails:ActivityFeed,textViewHeight:CGFloat){
        
        guard let currentFeed = feedDetails.feed else {return}
        
        if let username = currentFeed.userName{
            lblName.text = username
        }else{
            lblName.text = ""
        }
        
        if let feedContent = currentFeed.content{
            txtBody.text = feedContent
        }else{
            txtBody.text = ""
        }
            
        if let updatedDate = currentFeed.updatedDate {
        ...
        if let numberOfLikes = currentFeed.likecount{
            if numberOfLikes != 0{
                if countText != ""{
        ...
        if let unwrappedImage = currentFeed.image {
        ...
        imgProfile.image = UIImage(named: "default_profile")
        if let unwrappedUserImage = currentFeed.userPic {
